...
export default function Pdfsum() {
  ...
  const handleSubmit = async (e) => {
    ...
    for (let key in docxs) {
      formdata.append("docxs", docxs[key]);
    }
    formdata.append("name", name);
    const res = await axios.post(
      "https://doctotxtapi.onrender.com/api/v1/word/create",
      formdata
    );
    if (res.data.success) {
  ...
  return (
    <Layout>
      <form onSubmit={handleSubmit}>
        <label htmlFor="Docxs">Upload Word File</label>
        <input
          type="file"
          name="docxs"
          id="Docxs"
          multiple
          onChange={(e) => setdocxs(e.target.files)}
        />
